					    	...
					    	<tbody>
					    		<?php
									...
									function news($pageNum =1, $pageSize)
									{
										...
										$rs = "select suoshurenwuid,chuangjianrenpingfen,zuoyeneirong from zuoye where chengjierenemail='{$_SESSION["user"]}' order by chengjieshijian desc limit " . (($pageNum - 1) * $pageSize) . "," . $pageSize;
									...
									function allNews()
									{
									    ...
									    mysqli_select_db($coon, "web");
									    mysqli_set_charset($coon, "utf8");
									    $rs = "select count(*) num from zuoye where chengjierenemail='{$_SESSION["user"]}'"; //可以显示出总页数
									    $r = mysqli_query($coon, $rs);
								    ...
								    if(empty($array)==false)
					    			{
								    	...
								    	{	
								    		$renwusql = "select zhuti,renwuzhuangtai,jiezhishijian from renwu where id='{$values->suoshurenwuid}'";
								    		$renwur	= mysqli_query($coon, $renwusql);
								    		$renwuobj = mysqli_fetch_object($renwur);				    			
								    		echo "<tr>";								    	
								    		echo "<td><a href='/myweb/renwuGUI.php?id=$values->suoshurenwuid'>{$renwuobj->zhuti}";
								    		if($renwuobj->renwuzhuangtai==0)
								    			echo "<span style='color:green'>(未承接)</span></a></td>";
								    		if($renwuobj->renwuzhuangtai==1)
								    			echo "<span style='color:#FFC125'>(未完成)</span></a></td>";
								    		if($renwuobj->renwuzhuangtai==2)
								    			echo "<span>(已结束)</span></a></td>";
								    		if($renwuobj->renwuzhuangtai==3)
								    			echo "<span style='color:red'>(已过期)</span></a></td>";
								    		echo "<td>{$values->zuoyeneirong}</td>";
								    		echo "<td>{$renwuobj->jiezhishijian}</td>";
								    		echo "<td>{$values->chuangjianrenpingfen}</td>";
								    		echo "</tr>";
								    	}
								    }
								    else{
								    	echo "<tr>";
								    	echo "<td><h4>抱歉，暂无作业</h4></td>";
								    	echo "</tr>";	
								    	}		    			
					    		?>					    					    		
					    	</tbody>
