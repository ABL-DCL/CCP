					    	...
					    	<tbody>
					    		<?php
									...
									function news($pageNum =1, $pageSize)
									{
										...
										$rs = "select id,zhuti,fangxiangming,yonghuemail,chuangjianshijian,renwuzhuangtai from renwu where yonghuemail='{$_SESSION["user"]}' order by chuangjianshijian desc limit " . (($pageNum - 1) * $pageSize) . "," . $pageSize;
									...
									function allNews()
									{
									    ...
									    mysqli_select_db($coon, "web");
									    mysqli_set_charset($coon, "utf8");
									    $rs = "select count(*) num from renwu where yonghuemail='{$_SESSION["user"]}'"; //可以显示出总页数
									    $r = mysqli_query($coon, $rs);
								    ...
								    if(empty($array)==false)
					    			{
								    	...
								    	{							    			
								    		echo "<tr>";								    	
								    		echo "<td><a href='/myweb/renwuGUI.php?id=$values->id'>{$values->zhuti}";
