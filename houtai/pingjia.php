...
<?php
	...
	$suoshuzuoyeid = $_GET["zuoyeid"];
    	...
    	if($jianchaobj->num !=0 )
    	{
    		echo "<script>alert('请勿重复评价');window.location.href='/myweb/renwuGUI.php?id=$renwuid'</script>";
    		exit;
    	}
	...
	if($email == $yonghuemail)
	{
			...
			else
			{
				...
				echo "<script>alert('创建人打分成功');window.location.href='/myweb/renwuGUI.php?id=$renwuid'</script>";
			}
	...
	else
	{
		...
		if($obj->num != 0)
		{
			...
			else
			{
				...
				echo "<script>alert('评价成功');window.location.href='/myweb/renwuGUI.php?id=$renwuid'</script>";
			}
		}
		else
