...
<?php
	...
	if(!empty($_FILES['zyfujian']['tmp_name']))
	{
		...
		move_uploaded_file($_FILES['zyfujian']['tmp_name'], "/home/wwwroot/default/myweb/upfile/zuoye/{$tijiaoshijian}");
		$fujian = "/myweb/upfile/zuoye/{$tijiaoshijian}";
		$sql ="update zuoye set fujian='$fujian',tijiaoshijian='$tijiaoshijian',fujianming='$name' where id='$zuoyeid'";
		mysqli_query($coon, $sql);
		echo "<script>alert('上传成功！');window.location.href='/myweb/renwuGUI.php?id=$renwuid'</script>";
	}
	else
		echo "<script>alert('上传栏不能为空');window.location.href='/myweb/renwuGUI.php?id=$renwuid'</script>";
?>