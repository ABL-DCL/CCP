...
<?php
	...
	$chengjierenemail = $_SESSION["user"];
	$id = $_GET["id"];
	...
	if(isset($chengjierenemail))
	{
		if($chengjierenemail==$row1->yonghuemail)
			echo "<script>alert('你不能接受自己的任务');window.location.href='/myweb/renwuGUI.php?id=$renwuid'</script>";
		else{
			...
			else
			{
				...
				echo "<script>alert('接受作业成功！');window.location.href='/myweb/renwuGUI.php?id=$renwuid'</script>";			
			}
		}
	}
	else
		echo "<script>alert('请先登陆');window.location.href='/myweb/login.php'</script>";
?>